...
class Bomb(pygame.sprite.Sprite):
    ...
    @staticmethod
    def create_centers_of_bombs(field_size: tuple, bomb_size: tuple):
        center_width = bomb_size[0] // 2
        center_height = bomb_size[1] // 2
        centers = []

        while center_height < field_size[1]:
            while center_width < field_size[0]:
                centers.append((center_width, center_height))
                center_width += 2 * bomb_size[0] // 2
            center_height += 2 * bomb_size[1] // 2
            center_width = bomb_size[0] // 2
        return centers
